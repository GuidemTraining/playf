$(document).ready(function () {
    ...
    $(document).on('click', '.guidem-button', function() {
        ...
        if (form.length) {
            ...
            const sendSuccess = sendDataToServer(questionId, inputValue);

            // Check for sending data errors
            if (!sendSuccess) {
                showNotification('There has been something wrong with your request. Please contact Ian', 'red');
            }
        }
    });
    ...
    function sendDataToServer(questionId, answer) {
        ...
        if (!questionId || !userId || !lessonId || !answer) {
            // Show missing data notification
            showNotification('Missing data. Please contact Ian', 'red');
            return false; // Return false to indicate missing data
        }

        // Replace this with your actual logic to send data to the server
        // For now, simulate success with a random chance (50% chance of success)
