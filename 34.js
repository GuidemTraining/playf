$(document).ready(function () {
  const add_submit_btn = function () {
    ...
    // Loop through all elements with class "kapow-form"
    $(".kapow-form").each(function (index) {
      var $form = $(this);

      // Create a div with a submit button for each form
      ...
      if (isCompleted) {
        ...
        $(".kapow-submit-btn", $form).addClass("completed");
        $(".kapow-submit-btn", $form).text("Completed");
        $(".kapow-submit-btn", $form).prop("disabled", true);
        // Disable the input field and make it grayed out
        $("input[type='text']", $form).prop("disabled", true);
        $("input[type='text']", $form).css("background-color", "#ccc"); // Apply grayed-out style
      }
      ...
      // Add click event handler to the submit button for each form
      $(".kapow-submit-btn", $form).click(function () {
        if (isCorrectAnswer) {
          return; // Do nothing if the answer is already correct
        }

        var answer = $("input[type='text']", $form).val();
